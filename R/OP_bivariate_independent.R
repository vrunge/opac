...
OP_2D_1C <- function(data, beta = 4 * log(length(data$y1)))
{
  ...
  for(t in 2:n)
  {
    ...
    for(i in 1:nrow(info))
    {
      ...
      if(j == k){omega_t_k[ind] <- max(omega_t_k[ind], info$m[i])}
      else
      {
        #to do later, replace by only the test k > j
    ...
    ###
    ### STEP updating info with new data point y(t+1)
    ###
    ...
    for(i in 1:nrow(info)) # update m
    {
      ...
      else
      {
        ...
        info$m[i] <- eval_q_min(k, t+1) + (t-k+1)*(sqrt(D)-sqrt(R))^2
      }
    }
    ######### 2 ######### adding new 3-points with t
    i <- i + 1
    info[i, ] <- c(t, t, eval_q_min(t, t+1))
    for(k in indexSet)
    {
      i <- i + 1
      t1k <- (cumy1[t+1]-cumy1[k])/(t-k+1)
      t2k <- (cumy2[t+1]-cumy2[k])/(t-k+1)
      info[i, ] <- c(t, k, eval_q(k, t, t1k, t2k))
    }
