...
OP_2D_AC <- function(data, beta = 4 * log(nrow(data)))
{
  ...
  #########
  eval_q_21intersection <- function(i, j, k, t) ###value of m_{t}^{ijk}
  {
    Ri <- sqrt((costQ[shift(k-1)] - costQ[shift(i-1)])/(k-i) - eval_var(i,k-1))
    Rj <- sqrt((costQ[shift(k-1)] - costQ[shift(j-1)])/(k-j) - eval_var(j,k-1))
    t1i <- eval_meany1(i,k-1)
    ...
    t2A <- (1/2)*(t2i + t2j) - rho*(t2i - t2j) - mu*(t1i - t1j)
    mA <- eval_q(k, t, t1A, t2A)
    return(list(m = mA, p = c(t1A, t2A)))
  }

  #########
  eval_q_22intersection <- function(i, j, k, t) ###value of m_{t}^{ijk}
  {
    Ri <- sqrt((costQ[shift(k-1)] - costQ[shift(i-1)])/(k-i) - eval_var(i,k-1))
    Rj <- sqrt((costQ[shift(k-1)] - costQ[shift(j-1)])/(k-j) - eval_var(j,k-1))
    t1i <- eval_meany1(i,k-1)
  ...
  info3 <- data.frame(matrix(ncol = 11, nrow = 0)) ### info for pruning, triple points (m_t^{ijk})
  colnames(info3) <- c("k", "j", "i", "ti1", "ti2", "tj1", "tj2", "ijk1", "ijk2", "m", "type")
  info3[1,] <-  rep(0,11)
  ...
  for(t in 1:(n-1)) # at t, transform Q_t into Q_{t+1}
  {
    ...
    for(i in 1:nrow(info))
    {
      ...
      if(j == k)
      {
        ...
        for(l in indexSet)
        {
          if(eval_q(l,t,eval_meany1(k,t),eval_meany2(k,t)) < info$m[i]){test <- FALSE}
        }
        if(test == TRUE){omega_t_k[ind] <- max(omega_t_k[ind], info$m[i])}
      }
      else
      {
        t1k <- eval_meany1(k,t)
        ...
        {
          p <- position(j,k,t)
          if(eval_q(l,t,p[1],p[2]) < info$m[i]){test <- FALSE}
        }
        if(test == TRUE){if(eval_q_min(k, t) > eval_q(j, t, t1k, t2k)){omega_t_k[ind_k] <- max(omega_t_k[ind_k], info$m[i])}}
        ...
        if(test == TRUE){if(eval_q_min(j, t) > eval_q(k, t, t1j, t2j)){omega_t_k[ind_j] <- max(omega_t_k[ind_j], info$m[i])}}
      }
    }
    ...
    for(k in 1:length(indexSet))
    {
      if(omega_t_k[k] == -Inf){omega_t_k[k] <- Inf}
    }
    nonpruned <- NULL
    for(k in 1:length(indexSet))
    {
    ...
    if(nrow(info3) > 1)
    {
      for(l in 2:nrow(info3))
      {
        ...
        info3$tj1[l] <- position_j[1]
        info3$tj2[l] <- position_j[2]
        if(info3$type[l] == 1){res <- eval_q_21intersection(i,j,k,t+1)}
        if(info3$type[l] == 2){res <- eval_q_22intersection(i,j,k,t+1)}
        info3$m[l] <- res$m
      }
    }


    ###delete unseen points
    if(nrow(info3) > 1)
    {
      stay <- rep(TRUE, nrow(info3))
      for(l in 2:nrow(info3)) #remove row 1 = 0...0
      {
        if(info3$m[l] > eval_q(t+1,t+1,info3$ijk1[l],info3$ijk2[l])){stay[l] <- FALSE}
      }
      info3 <- info3[stay, ]
    }
    ...
    {
      inclus <- FALSE
      for(k in indexSet) # remove case of one disk included in one disk
      {
        if(j != k){if(test_inclusion(j, k, t+1)){inclus <- TRUE}}
      }
      if(inclus == FALSE){info <- rbind(info, c(t+1, j, eval_q_intersection(j, t+1, t+1)))}
    ...
    for(i in indexSet) #m_{t+1}^(ij(t+1)) optimization under two constraints
    {
      for(j in indexSet)
      {
        if(i< j)
        {
          res <- eval_q_21intersection(i,j,t+1,t+1)
          create <- TRUE
          for(m in indexSet)
          {
            if((m != i) && (m != j))
            {if(eval_q(m, t+1, res$p[1], res$p[2]) < res$m){create <- FALSE}}
          }
          if(create == TRUE)
          {
            info3 <- rbind(info3, c(t+1, j, i,
                                      position(i,t+1,t+1),
                                      position(j,t+1,t+1),
                                      res$p, res$m, 1))
          }
          res <- eval_q_22intersection(i,j,t+1,t+1)
          create <- TRUE
          for(m in indexSet)
          {
            if((m != i) && (m != j))
