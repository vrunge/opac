...
#' dataGenerator2D
#' @description Generate data for changes in bivariate independent time series
#' @param chpts a vector of increasing changepoint indices
...
#' @return a dataframe with two simulated time series of same length
#' @examples
#' myData <- dataGenerator2D(chpts = c(30,100,120), means1 = c(0,5,0), means2 = c(7,1,-4))
dataGenerator2D <- function(chpts = c(50, 100),
                            means1 = c(0,5),
                            means2 = c(3,0),
                            sdNoise1 = 1,
                            sdNoise2 = 1)
...
#' @param meansX mean values for X values generated by a Gaussian model
#' @param sdX standard deviation values for X values generated by a Gaussian model
#' @param sdNoise standard deviation of the Gaussian noise
#' @return a dataframe with time series x and y for change-points in regression of type y = Ax + B + noise
#' @examples
#' myData <- dataGeneratorRegression(chpts = c(40,90))
dataGeneratorRegression <- function(chpts = c(50,150),
                                    A = c(2,1),
                                    B = c(-1,2),
                                    meansX = 0,
                                    sdX = 1,
                                    sdNoise = 1)
{
  ...
  if(sdX < 0){stop('varX cannot be negative')}
...
#' @param chpts a vector of increasing changepoint indices
#' @param kinks vector of successive kink values (kink height)
...
#' @examples
#' myData <- dataGeneratorSlope(c(1,30,100,150), c(-2,pi,0.001,2.1), 0.1)
dataGeneratorSlope <- function(chpts, kinks, varNoise = 0)
{
  ############
  ### STOP ###
  ############
